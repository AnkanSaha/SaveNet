...
document
  ...
  .addEventListener('click', updateProfileMenubar)
if (window.location.pathname == '/dashboard') {
  document
    .getElementById('dashboard-userprofile-option')
    .addEventListener('click', updateProfileMenubar)
}
...
function updateProfileMenubar () {
  ...
  const AccountCreateDate = localStorage.getItem('AccountCreateDate')
...
document
  ...
  .addEventListener('click', validator)
...
function validator () {
  ...
  const AccountCreateDate = document.getElementById('AccountCreateDate').value
  const Password = document.getElementById('password').value
  const Account_Create_Time_Device_Info = {
  ...
  if (navigator.onLine == false) {
    alert(
      'You are offline, please connect to the internet to update your profile'
    )
  } else if (
    Name == '' ||
    Email == '' ||
...
async function updater (
  ...
  if (data.status == 'Account Info Updated') {
    ...
    setTimeout(() => {
      document.getElementById('maincontainer').innerHTML =
        `<h1 class="mb-4 text-center text-lg font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-5xl dark:text-white lg:mt-[11.25rem] mt-[15.25rem]">${data.status}</h1>`
    }, 1000)
