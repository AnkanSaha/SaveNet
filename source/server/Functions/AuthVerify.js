...
async function Registration (Name, Email, Country, Password, res, Device_info) {
  ...
  try {
    ...
    const Registration = new AuthModel.AuthScheema(tempdata)
    try {
      const result = await AuthModel.AuthScheema.find({ Email })
      console.log(result)
      if (result == '') {
        try {
          const AccountID = await AuthModel.AuthScheema.find({
            Account_ID: UniqueAccountID
          })
...
async function updateAccountInfo (
  ...
  await mongoose.connect(GeneralData.MONGO_URI)
  try {
    const result = await AuthModel.AuthScheema.find({ Account_ID: AccountID })
    if (result.length == 0) {
      res.status(404).json({ status: 'User Not Registered' })
    } else if (result.length != 0) {
      const resp = await AuthModel.AuthScheema.findOneAndUpdate(
        { Account_ID: AccountID },
        {
          Account_ID: AccountID,
          Name,
      ...
      res.status(200).json({
        status: 'Account Info Updated',
        AccountID: resp.Account_ID,
        AccountCreateDate: resp.Account_Create_Date,
        Name: resp.Name,
        Email: resp.Email,
