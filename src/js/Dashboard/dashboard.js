...
async function FetchTitle() {
  ...
  document.getElementById(
    "titles"
  ).innerHTML = `<div class="flex flex-col items-center justify-center w-full h-full mt-[7.25rem] lg:mt-[5.25rem]">No Titles Available ...</div>`;
    ...
    data.titles.forEach((selected) => {
      ...
      id.forEach((selected) => {
        selected.addEventListener("click", () => {
          ...
          document.getElementById("descriptiontemplate").innerHTML =
            DescriptionLoading;
          var Attname = selected.getAttribute("name");
          fetch("/getdatainfo", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ Title: Attname, Email: Email }),
          })
            .then((res) => res.json())
            .then((data) => {
