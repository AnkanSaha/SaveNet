...
function showAlltitles() {
  ...
  caches.open("titleDetails").then(async (cache) => {
    var TitleNameres = await cache.match("/Alltitles");
    var TitleID = await cache.match("/AlltitlesID");
    if (TitleNameres) {
      ...
      var resp = await cache.match("/Alltitles");
      if (resp) {
    ...
    document.querySelectorAll(".delete-btn").forEach((selectedbtn) => {
      selectedbtn.addEventListener("click", (event) => {
        ...
        if (online_status == true) {
          var Selected_Button_title = event.target.getAttribute("name");
...
async function DeleteTitle(ID) {
  var res = await fetch("/deletedata", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ DataID: ID }),
  });
  var DeleteResponsedata = await res.json();
  if (DeleteResponsedata.Status == "Success") {
    await caches.delete("titleDetails");
    gettotalTilteLength();
    alert("Data Deleted successfully from the database");
    document.getElementById("maincontainer").innerHTML = `<h1 class="mb-4 text-center text-lg font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-5xl dark:text-white lg:mt-[11.25rem] mt-[15.25rem]">Data deleted successfully</h1>`;
