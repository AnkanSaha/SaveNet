...
document.getElementById("dashboard-overview").addEventListener("click", overview);

async function overview(){
    document.getElementById("maincontainer").innerHTML = "";
    const overview_design = `<p style="font-family: 'Sofia Sans Extra Condensed', sans-serif;" class="text-2xl mt-[2rem] ml-[1.75rem] lg:ml-[2.25rem] lg:text-6xl text-gray-900 dark:text-white">Wellcome ${Name}</p>
    ...
    document.getElementById("maincontainer").insertAdjacentHTML('beforeend', Sheleton_loading_animation)
        ...
        if(res){
            ...
            Main_data_Title.forEach((data,index)=>{
                ...
                <p class="font-normal text-gray-700 dark:text-gray-400">Added By Ankan Saha.</p>
              </a>`
            })
            ...
            Main_data_Title.forEach((data,index)=>{
                overview_data_HTML_Template+=`
                <a DataID="${Main_Data_ID[index]}" class="read-btn block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">${data}</h5>
                <p class="font-normal text-gray-700 dark:text-gray-400">Added By Ankan Saha.</p>
              </a>`
            })
